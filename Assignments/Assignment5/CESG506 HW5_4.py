...
for i in range(0, nrNodes):
    if i == 0:
        ...
        p5.addNode(i + 1, array([1, 0, 1]), array([x[i], y[i]]))
    else:
...
# Assign loads
p5.addLoad(1, nrNodes, array([-1., 0., 0.]))
...
upath, load, s_hist, R_hist, det_hist = p5.solveArc(load=6, s_lim=1000, la=0.2, alfa=0.0, data=True)
...
plt.title(f'Homework 5-4, {nrEle} elements curved beam')
